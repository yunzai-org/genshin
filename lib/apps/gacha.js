...
class gacha extends Plugin {
    ...
    async gacha() {
        ...
        await this.renderImg('genshin', 'html/gacha/gacha-trial', data, {
            recallMsg: data.nowFive >= 1 || data.nowFour >= 4
                ? false
    ...
    checkLimit() {
        ...
        let nowCount = num;
        if (this.GachaData.type == 'weapon')
            nowCount = weaponNum;
        if (this.GachaData.set.LimitSeparate == 1) {
            if (nowCount < this.GachaData.set.count * 10)
                return false;
        }
        else {
            if (num + weaponNum < this.GachaData.set.count * 10)
                return false;
        }
