...
class gcLog extends Plugin {
    ...
    helpPort() {
        ...
        if (['电脑', 'pc'].includes(msg)) {
            this.e.reply(global.segment.image(`file://${_path}/resources/logHelp/记录帮助-电脑.png`));
        }
        else if (['安卓'].includes(msg)) {
            this.e.reply(`安卓抽卡记录获取教程：${androidUrl}`);
        }
        else if (['苹果', 'ios'].includes(msg)) {
            this.e.reply(global.segment.image(`file://${_path}/resources/logHelp/记录帮助-苹果.png`));
        }
    }
    async logCount() {
