...
class payLog extends Plugin {
    ...
    async getAuthKey() {
        ...
        await global.redis.setEx(`Yz:genshin:mys:qq-uid:${this.e.user_id}`, 3600 * 24 * 30, imgData.uid);
        await global.redis.setEx(`Yz:genshin:payLog:${imgData.uid}`, 3600 * 24, this.authKey);
        return true;
    }
        ...
        if (uid) {
            ...
            if (this.authKey) {
                ...
                this.reply([
                    ...
                    global.segment.button([{ text: '充值帮助', callback: '#充值统计帮助' }])
                ]);
            }
        }
        else {
