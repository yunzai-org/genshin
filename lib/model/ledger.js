...
import { MysInfo, MysApi, GSCfg } from 'yunzai-mys';
...
class Ledger extends base {
    ...
    dealData(ledgerInfo) {
        ...
        let icon = '';
        if (this.e.isSr)
            icon = lodash.sample(fs
                .readdirSync(`${this._path}/plugins/genshin/resources/StarRail/img/role`)
    ...
    async ysLedger(ck, month, isTask) {
        ...
        if (!ledgerInfo || ledgerInfo.retcode != 0)
            return false;
