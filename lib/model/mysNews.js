...
import { puppeteer, sleep } from 'yunzai';
...
class MysNews extends base {
    ...
    async getNews(gid) {
        ...
        if (this.e.msg.includes('列表')) {
            ...
            const page = this.e.msg
                .replace(/#|＃|官方|星铁|原神|崩坏三|崩三|绝区零|崩坏二|崩二|崩坏学园二|未定|未定事件簿|公告|资讯|活动/g, '')
                .trim() || 1;
            if (page > data.length) {
    ...
    async detalData(data, gid) {
        ...
        if (typeof json == 'object') {
            ...
            for (const img of data.post.images) {
                data.post.content = data.post.content.replace(img, img +
    ...
    async mysEstimate(keyword, uid) {
        ...
        const img = await this.render(param);
        if (img.length > 1) {
            img.push(segment.image(param.data.post.images[0] +
                '?x-oss-process=image//resize,s_600/quality,q_80/auto-orient,0/interlace,1/format,jpg'));
        }
        return this.replyMsg(img, `${param.data.post.subject}`);
    }
    ...
    async ActivityPush() {
        ...
        if ((!pushGroupList.gsActivityPush || pushGroupList.gsActivityPush == {}) &&
        ...
        for (let item of BotidList) {
            ...
            if (!Array.isArray(redisapgl.GroupList) ||
            ...
            for (let a of ActivityList) {
                ...
                Bot[item]
                    ...
                    .sendMsg(msgList)
                    .then(() => { })
                    .catch(err => logger.error(`[米游社活动到期推送] ${item}:${redisapgl.GroupList[0]} 推送失败，错误信息${err}`));
            }
            redisapgl.GroupList.shift();
            await redis.set(`Yz:apgl:${item}`, JSON.stringify(redisapgl));
        }
    ...
    async sendNews(botId, groupId, typeName, postId, gid) {
        if (!this.pushGroup[groupId])
            this.pushGroup[groupId] = 0;
        if (this.pushGroup[groupId] >= this.maxNum)
            return;
