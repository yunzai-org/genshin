...
import { GSCfg, MysInfo } from 'yunzai-mys';
...
class RoleIndex extends base {
    ...
    async getIndex() {
        ...
        if (!res || res[0].retcode !== 0 || res[2].retcode !== 0)
            return false;
        let ret = [];
    ...
    dealData(data) {
        ...
        let percentage = lodash.round(((stats.precious_chest_number +
            ...
            this.all_chest) *
            100, 1);
        let afterPercentage = (percentage < 60
            ? 'D'
            : percentage < 70
                ? 'C'
                : percentage < 80
        ...
        let line = [
            [
                ...
                },
                {
                    lable: '获取率',
                    num: afterPercentage,
                    color: afterPercentage.substr(0, 1) == 'D'
                        ? '#12a182'
            ...
            : (percentage < 70
                ...
                : percentage < 80
                    ...
                    : percentage < 90
                        ? 'A'
                        : 'S') + `[${percentage}%]`;
        let daysDifference = Math.floor((Date.now() - new Date('2020-09-15').getTime()) / (1000 * 60 * 60 * 24)) + 1;
        let line = [
            [
                { lable: '角色数', num: stats.avatar_number, extra: this.lable.avatar },
                {
