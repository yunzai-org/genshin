...
import { NoteUser, GSCfg, MysUser, sequelize, UserGameDB, MysInfo } from 'yunzai-mys';
import { promisify } from 'node:util';
import YAML from 'yaml';
import { Player, Data } from '../miao.js';
...
class User extends base {
    ...
    async bing() {
        ...
        let button = [];
        if (mys.hasGame('gs')) {
            msg.push('原神模块支持：', '【#uid】当前绑定ck uid列表', '【#我的ck】查看当前绑定ck', '【#删除ck】删除当前绑定ck', '【#体力】查询当前树脂', '【#原石】查看原石札记', '【#原石统计】原石统计数据', '【#练度统计】角色列表数据', '【#面板】【#更新面板】面板信息');
            button.push([
        ...
        if (mys.hasGame('sr')) {
            msg.push('星穹铁道支持：', '【*uid】当前绑定ck uid列表', '【*删除ck】删除当前绑定ck', '【*体力】查询当前开拓力', '【*星琼】查看星琼月历', '【*星琼统计】星琼统计数据', '【*练度统计】角色列表数据', '【*面板】【*更新面板】面板信息');
            button.push([
                { text: '*uid', callback: '*uid' },
                { text: '*删除ck', callback: '*删除ck' },
                { text: '*体力', callback: '*体力' }
        ...
        msg = await makeForwardMsg(this.e, [[msg.join('\n'), segment.button(...button)]], '绑定成功：使用命令说明');
    ...
    async showUid() {
        ...
        const img = segment.image(await this.e.runtime.render('genshin', 'html/user/uid-list', { uids }));
        return this.e.reply([
            img,
            segment.button([
    ...
    async bindNoteUser() {
        ...
        if (idRet && idRet[1]) {
            ...
            if (!idRet[2]) {
                ...
                await global.redis.set(`Yz:NoteUser:verify:${mainId}`, verify + '||' + currId, { EX: 300 });
                e.reply([
